...
namespace Microsoft.AspNetCore.Razor.Evolution.SyntaxTreePhase.Parser
{
    public class HtmlMarkupParser : TokenizerBackedParser<HtmlTokenizer, HtmlSymbol, HtmlSymbolType>
    {
        ...
        private readonly ParserBase _codeParser;

        public HtmlMarkupParser(ParserBase codeParser, ParserContext context)
                ...
                using (Context.Builder.StartBlock(BlockType.Markup))
                {
                    ...
                    else
                    {
                        Context.ErrorSink.OnError(
                            CurrentSymbol.Start,
                            Resource.ParseError_MarkupBlock_Must_Start_With_Tag,
                                   ...
                                   IDisposable tagBlockWrapper)
        {
            ...
            if (tags.Count == 0)
            {
                Context.ErrorSink.OnError(
                    CurrentLocation,
                    Resource.ParseError_OuterTagMissingName,
        ...
        private bool EndTextTag(HtmlSymbol solidus, IDisposable tagBlockWrapper)
        {
            ...
            if (!seenCloseAngle)
            {
                Context.ErrorSink.OnError(
                    textLocation,
                    Resource.ParseError_TextTagCannotContainAttributes,
                    length: 4 /* text */);
                               ...
                               IDisposable tagBlockWrapper)
        {
            ...
            if (!seenClose)
            {
                Context.ErrorSink.OnError(
                    ...
                    Resource.FormatParseError_UnfinishedTag(tag.Item1.Content),
                    ...
                    using (Context.Builder.StartBlock(BlockType.Tag))
                    {
                        ...
                        if (!Optional(HtmlSymbolType.CloseAngle))
                        {
                            Context.ErrorSink.OnError(
                                ...
                                Resource.FormatParseError_UnfinishedTag(ScriptTagName),
        ...
        private bool RemoveTag(Stack<Tuple<HtmlSymbol, SourceLocation>> tags, string tagName, SourceLocation tagStart)
        {
            ...
            if (currentTag != null)
            {
                Context.ErrorSink.OnError(
                    ...
                    Resource.FormatParseError_MissingEndTag(currentTag.Item1.Content),
            ...
            else
            {
                Context.ErrorSink.OnError(
                    SourceLocation.Advance(tagStart, "</"),
                    Resource.FormatParseError_UnexpectedEndTag(tagName),
                    tagName.Length);
            }
            return false;
        }
        ...
        private void EndTagBlock(Stack<Tuple<HtmlSymbol, SourceLocation>> tags, bool complete)
        {
            if (tags.Count > 0)
            {
                ...
                Context.ErrorSink.OnError(
                    SourceLocation.Advance(tag.Item2, "<"),
                    Resource.FormatParseError_MissingEndTag(tag.Item1.Content),
