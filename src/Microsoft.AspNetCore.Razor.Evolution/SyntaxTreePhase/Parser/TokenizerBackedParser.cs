...
namespace Microsoft.AspNetCore.Razor.Evolution.SyntaxTreePhase.Parser
{
        ...
        where TSymbol : SymbolBase<TSymbolType>
    {
        ...
        protected internal bool Balance(BalancingModes mode)
        {
            ...
            if (EndOfFile && ((mode & BalancingModes.NoErrorOnFailure) != BalancingModes.NoErrorOnFailure))
            {
                Context.ErrorSink.OnError(
                    start,
                    Resource.FormatParseError_Expected_CloseBracket_Before_EOF(
        ...
        {
            if (Span.Symbols.Count == 0 && Context.Builder.LastSpan.EditHandler.AcceptedCharacters != AcceptedCharacters.Any)
            {
        ...
        protected internal bool Required(TSymbolType expected, bool errorIfNotFound, Func<string, string> errorBase)
        {
            ...
            if (!found && errorIfNotFound)
            {
                ...
                {
                    error = Resource.ErrorComponent_Newline;
                }
                else if (Language.IsWhiteSpace(CurrentSymbol))
                {
                    error = Resource.ErrorComponent_Whitespace;
                }
                else if (EndOfFile)
                {
                    error = Resource.ErrorComponent_EndOfFile;
                }
                else
                {
                    error = Resource.FormatErrorComponent_Character(CurrentSymbol.Content);
                }
        ...
        {
            throw new InvalidOperationException(Resource.Language_Does_Not_Support_RazorComment);
        }
        ...
        protected void RazorComment()
        {
            ...
            using (PushSpanConfig(CommentSpanConfig))
            {
                ...
                {
                    Context.Builder.CurrentBlock.ChunkGenerator = ParentChunkGenerator.Null;
                    var start = CurrentLocation;
                    ...
                    if (!Optional(KnownSymbolType.CommentStar))
                    {
                        ...
                        Context.ErrorSink.OnError(
                            start,
                            Resource.ParseError_RazorComment_Not_Terminated,
                            length: 2 /* @* */);
                    }
                    else
                    {
