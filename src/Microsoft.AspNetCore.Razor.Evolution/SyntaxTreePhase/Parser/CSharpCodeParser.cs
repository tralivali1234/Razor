...
namespace Microsoft.AspNetCore.Razor.Evolution.SyntaxTreePhase.Parser
{
    public class CSharpCodeParser : TokenizerBackedParser<CSharpTokenizer, CSharpSymbol, CSharpSymbolType>
    {
        ...
        private readonly HtmlMarkupParser _htmlParser;

        public CSharpCodeParser(HtmlMarkupParser htmlParser, ParserContext context)
        ...
        public override void ParseBlock()
        {
            using (PushSpanConfig(DefaultSpanConfig))
            {
                if (Context == null)
                {
                    throw new InvalidOperationException(Resource.Parser_Context_Not_Set);
                }
            ...
            using (PushSpanConfig(DefaultSpanConfig))
            {
                ...
                try
                {
                            ...
                            else
                            {
                                    ...
                                    StringComparison.Ordinal))
                                {
                                    Context.ErrorSink.OnError(
                                        CurrentLocation,
                                        Resource.FormatParseError_HelperDirectiveNotAvailable(
                                            SyntaxConstants.CSharp.HelperKeyword),
                    ...
                    if (At(CSharpSymbolType.WhiteSpace) || At(CSharpSymbolType.NewLine))
                    {
                        Context.ErrorSink.OnError(
                            CurrentLocation,
                            Resource.ParseError_Unexpected_WhiteSpace_At_Start_Of_CodeBlock_CS,
                            CurrentSymbol.Content.Length);
                    }
                    else if (EndOfFile)
                    {
                        Context.ErrorSink.OnError(
                            CurrentLocation,
                            Resource.ParseError_Unexpected_EndOfFile_At_Start_Of_CodeBlock,
                    ...
                    else
                    {
                        Context.ErrorSink.OnError(
                            CurrentLocation,
                            Resource.FormatParseError_Unexpected_Character_At_Start_Of_CodeBlock_CS(
                                CurrentSymbol.Content),
                            CurrentSymbol.Content.Length);
                    }
                }
        ...
        private void VerbatimBlock()
        {
            ...
            var block = new Block(Resource.BlockName_Code, CurrentLocation);
            AcceptAndMoveNext();
        ...
        {
            if (insertMarkerIfNecessary && Context.Builder.LastSpan.EditHandler.AcceptedCharacters != AcceptedCharacters.Any)
            {
                AddMarkerSymbolIfNecessary();
            }
        ...
        {
            var block = new Block(Resource.BlockName_ExplicitExpression, CurrentLocation);
            ...
            using (PushSpanConfig(ConfigureExplicitExpressionSpan))
            {
                ...
                if (!success)
                {
                    ...
                    Context.ErrorSink.OnError(
                        block.Start,
                        Resource.FormatParseError_Expected_EndOfBlock_Before_EOF(block.Name, ")", "("),
                        length: 1 /* ( */);
                }
        ...
        private void Template()
        {
            if (Context.Builder.ActiveBlocks.Any(block => block.Type == BlockType.Template))
            {
                Context.ErrorSink.OnError(
                    CurrentLocation,
                    Resource.ParseError_InlineMarkup_Blocks_Cannot_Be_Nested,
                    length: 1 /* @ */);
            }
            Output(SpanKind.Code);
        ...
        protected virtual void ReservedDirective(bool topLevel)
        {
            Context.ErrorSink.OnError(
                CurrentLocation,
                Resource.FormatParseError_ReservedWord(CurrentSymbol.Content),
                CurrentSymbol.Content.Length);
            AcceptAndMoveNext();
            Span.EditHandler.AcceptedCharacters = AcceptedCharacters.None;
            Span.ChunkGenerator = SpanChunkGenerator.Null;
        ...
        private void UsingKeyword(bool topLevel)
        {
            ...
            else if (At(CSharpSymbolType.Identifier) || At(CSharpKeyword.Static))
            {
                ...
                if (!topLevel)
                {
                    Context.ErrorSink.OnError(
                        block.Start,
                        Resource.ParseError_NamespaceImportAndTypeAlias_Cannot_Exist_Within_CodeBlock,
                        block.Name.Length);
                    StandardStatement();
                }
                else
        ...
        private void ExpectCodeBlock(Block block)
        {
            if (!EndOfFile)
            {
                ...
                if (!At(CSharpSymbolType.LeftBrace))
                {
                    Context.ErrorSink.OnError(
                        CurrentLocation,
                        Resource.FormatParseError_SingleLine_ControlFlowStatements_Not_Allowed(
        ...
        private void Statement(Block block)
        {
            ...
            if (isMarkup)
            {
                if (type == CSharpSymbolType.Transition && !isSingleLineMarkup)
                {
                    Context.ErrorSink.OnError(
                        loc,
                        Resource.ParseError_AtInCode_Must_Be_Followed_By_Colon_Paren_Or_Identifier_Start,
                        length: 1 /* @ */);
                }
        ...
        private void HandleStatement(Block block, CSharpSymbolType type)
        {
            switch (type)
            {
                ...
                case CSharpSymbolType.LeftBrace:
                    // Verbatim Block
                    block = block ?? new Block(Resource.BlockName_Code, CurrentLocation);
                    AcceptAndMoveNext();
                    CodeBlock(block);
                    break;
        ...
        private void EmbeddedExpression()
        {
            ...
            else
            {
                ...
                if (At(CSharpSymbolType.LeftBrace))
                {
                    Context.ErrorSink.OnError(
                        CurrentLocation,
                        Resource.ParseError_Unexpected_Nested_CodeBlock,
                        length: 1  /* { */);
                }
        ...
        {
            var nested = Context.Builder.ActiveBlocks.Any(block => block.Type == BlockType.Template);
            ...
            if (nested)
            {
                Context.ErrorSink.OnError(
                    startLocation,
                    Resource.FormatParseError_Sections_Cannot_Be_Nested(Resource.SectionExample_CS),
            ...
            if (!Required(CSharpSymbolType.Identifier,
                          ...
                          errorBase: Resource.FormatParseError_Unexpected_Character_At_Section_Name_Start))
            {
                if (!errorReported)
                {
                    errorReported = true;
            ...
            if (!sawStartingBrace)
            {
                if (!errorReported)
                {
                    ...
                    Context.ErrorSink.OnError(
                        errorLocation,
                        Resource.ParseError_MissingOpenBraceAfterSection,
                        length: 1  /* { */);
                }
            ...
            if (!Optional(CSharpSymbolType.RightBrace))
            {
                ...
                Context.ErrorSink.OnError(
                    ...
                    Resource.FormatParseError_Expected_EndOfBlock_Before_EOF(
        ...
        protected virtual void FunctionsDirective()
        {
            ...
            if (!At(CSharpSymbolType.LeftBrace))
            {
                Context.ErrorSink.OnError(
                    CurrentLocation,
                    Resource.FormatParseError_Expected_X(Language.GetSample(CSharpSymbolType.LeftBrace)),
                    length: 1 /* { */);
                CompleteBlock();
                Output(SpanKind.MetaCode);
                return;
        ...
        {
            BaseTypeDirective(
                Resource.ParseError_InheritsKeyword_Must_Be_Followed_By_TypeName,
        ...
        private void TagHelperDirective(string keyword, Func<string, ISpanChunkGenerator> chunkGeneratorFactory)
        {
            ...
            if (EndOfFile || At(CSharpSymbolType.NewLine))
            {
                Context.ErrorSink.OnError(
                    keywordStartLocation,
                    Resource.FormatParseError_DirectiveMustHaveValue(keyword),
                    keywordLength);
            ...
            else
            {
                ...
                if (startsWithQuote != endsWithQuote)
                {
                    Context.ErrorSink.OnError(
                        startLocation,
                        Resource.FormatParseError_IncompleteQuotesAroundDirective(keyword),
                        rawValue.Length);
                }
